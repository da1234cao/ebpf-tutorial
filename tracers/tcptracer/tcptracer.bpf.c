...
static __always_inline bool filter_event(struct sock *sk, __u32 uid,
                                         __u32 pid) {
...
static __always_inline int exit_tcp_connect(int ret, __u16 family) {
  __u64 pid_tgid = bpf_get_current_pid_tgid();
  __u32 pid = pid_tgid >> 32;
  __u32 tid = pid_tgid;
  __u64 uid_gid = bpf_get_current_uid_gid();
  ...
  bpf_printk("exit_tcp_connect");
  if (!fill_tuple(&tuple, sk, family))
    goto end;
  bpf_printk("tuple:%d:%d<-->%d:%d", tuple.saddr_v4, tuple.sport,
             tuple.daddr_v4, tuple.dport);
